...
# Zoek de laatst opgeslagen EWS_kern en EWS_percelen in output-folder
previous_files <- list.files(output_dir, pattern = "EWS_.*\\.gpkg$", full.names = TRUE)

if (length(previous_files) >= 2) {
  message("Vorige datasets gevonden, laad oude bestanden in...")
  
  # Oude datasets laden
  EWS_kern_old <- st_read(previous_files[grepl("EWS_kern", previous_files)], quiet = TRUE)
  EWS_percelen_old <- st_read(previous_files[grepl("EWS_percelen", previous_files)], quiet = TRUE)
